...
class AIManager
{
    public function generateWithMemory(
        string $prompt,
        string $sessionId,
        ...
        try {
            ...
            $conversation = $memory->getOrCreateConversation($sessionId, $tenantId);
            $memory->addMessage($conversation, 'user', $prompt);
            $history = $memory->getHistory($conversation);

            $context  = $tenantId ? $this->getRagContext($prompt, $tenantId) : '';
            $messages = $this->buildMessages($history, $systemPrompt, $context);
    ...
    private function getRagContext(string $prompt, int $tenantId): string
    {
        try {
            return app(RagService::class)->getContext($prompt, $tenantId);
        } catch (\Throwable) {
            return '';
        }
    }
    ...
    private function buildMessages(array $history, ?string $systemPrompt, string $context): array
    {
        $system = $systemPrompt ?? config('ai.health_system_prompt');

        if ($context !== '') {
            $system .= "\n\nUse the following health document context to inform your response:\n\n" . $context;
        }
