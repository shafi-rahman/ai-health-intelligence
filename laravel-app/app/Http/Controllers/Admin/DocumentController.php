...
class DocumentController extends Controller
{
    public function index(Request $request)
    {
        $docs = Document::where('tenant_id', $request->user()->tenant_id)
            ->latest()
            ->get(['id', 'title', 'type', 'category', 'status', 'chunk_count', 'created_at']);

        return response()->json($docs);
    }

    private const FILE_TYPES  = ['pdf', 'docx', 'csv', 'txt'];
    private const IMAGE_TYPES = ['image'];

    public function store(Request $request)
    {
        $request->validate([
            'type'     => 'required|in:pdf,docx,csv,txt,text,url,image',
            'category' => 'nullable|in:general,medical_report,prescription',
            ...
            'file'     => [
                ...
                \Illuminate\Validation\Rule::requiredIf(
                    in_array($request->type, array_merge(self::FILE_TYPES, self::IMAGE_TYPES))
                ),
                'mimes:pdf,docx,csv,txt,jpg,jpeg,png,webp',
            ],
        ]);
        ...
        if (in_array($request->type, array_merge(self::FILE_TYPES, self::IMAGE_TYPES))) {
    ...
    public function analysis(Request $request, int $id)
    {
        ...
        if (is_null($document->analysis_result)) {
            return response()->json([
                'message' => $document->status === 'ready'
                    ? 'No analysis available. This document category may not support analysis.'
